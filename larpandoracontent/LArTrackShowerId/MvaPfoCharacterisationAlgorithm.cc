...
bool MvaPfoCharacterisationAlgorithm<T>::IsClearTrack(const Cluster *const pCluster) const
{
    ...
    {
        return LArMvaHelper::Classify(m_multiVariateAnalysis, featureVector);
    }
    else
    {
        return (LArMvaHelper::CalculateProbability(m_multiVariateAnalysis, featureVector) > m_minProbabilityCut);
    }
}
...
bool MvaPfoCharacterisationAlgorithm<T>::IsClearTrack(const pandora::ParticleFlowObject *const pPfo) const
{
    ...
    //if no failures, proceed with mva classification
    if (!m_enableProbability)
    {
        return LArMvaHelper::Classify((wClusterList.empty() ? m_multiVariateAnalysisNoChargeInfo : m_multiVariateAnalysis), featureVector);
    }
    else
    {
        const double score(LArMvaHelper::CalculateProbability((wClusterList.empty() ? m_multiVariateAnalysisNoChargeInfo : m_multiVariateAnalysis), featureVector));
        object_creation::ParticleFlowObject::Metadata metadata;
...
StatusCode MvaPfoCharacterisationAlgorithm<T>::ReadSettings(const TiXmlHandle xmlHandle)
{
    ...
    {
        PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
            "MvaFileNameNoChargeInfo", m_mvaFileNameNoChargeInfo));
    ...
    else
    {
        ...
        m_multiVariateAnalysis.Initialize(fullMvaFileName, m_mvaName);

        if (m_useThreeDInformation)
        {
            if (m_mvaFileNameNoChargeInfo.empty() || m_mvaNameNoChargeInfo.empty())
            {
                std::cout << "MvaPfoCharacterisationAlgorithm: MvaFileName and MvaName must be set if in classification mode for no charge info in 3D mode " << std::endl;
                return STATUS_CODE_INVALID_PARAMETER;
            }
            ...
            m_multiVariateAnalysisNoChargeInfo.Initialize(fullMvaFileNameNoChargeInfo, m_mvaNameNoChargeInfo);
        }
    }
